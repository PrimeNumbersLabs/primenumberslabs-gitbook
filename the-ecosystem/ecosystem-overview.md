# Ecosystem Overview

The Prime Numbers ecosystem is built around **three core protocols** that reinforce each other.

No protocol operates in isolation, **fees, liquidity, and rewards flow between all three**, ultimately returning value to **$PRFI holders**.

***

#### <mark style="color:purple;">PrimeFi: Omnichain Lending & Borrowing</mark>

PrimeFi enables users to deposit collateral and borrow assets across multiple chains.

* Cross-chain lending and borrowing
* Available on XDC, Hyperliquid, and Base
* A share of borrowing fees flows to PRFI NFT holders

→ Full documentation: [https://docs.primefi.xyz](https://docs.primefi.xyz/)

***

#### <mark style="color:purple;">PrimeStaking: Liquid Staking & NFT Vaults</mark>

PrimeStaking allows users to stake XDC while maintaining liquidity.

* Receive psXDC at a 1:1 ratio
* Staking positions are wrapped into NFTs
* Positions are transferable and tradable
* A share of staking rewards flows to PRFI NFT holders

→ Full documentation: [https://docs.primestaking.xyz](https://docs.primestaking.xyz/)

***

#### <mark style="color:purple;">PrimePort: NFT Marketplace</mark>

PrimePort is the marketplace layer of the ecosystem.

* Create, buy, sell, and auction NFTs across EVM chains
* Secondary market for staking positions
* 1.5% fee on secondary sales

A portion of marketplace fees flows to PRFI NFT holders.

→ Full documentation: [PrimePort Docs](https://docs.primeport.xyz/)

***

#### <mark style="color:purple;">$PRFI: The Ecosystem Token</mark>

$PRFI is the **utility and governance token** that connects all protocols.

* Staked inside PRFI NFTs
* Grants access to ecosystem revenue
* Receives a share of fees from:
  * PrimeFi
  * PrimeStaking
  * PrimePort

More details in the [**$PRFI Token** section.](https://docs.primenumbers.xyz/usdprfi-token)
